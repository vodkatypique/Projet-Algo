...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Questions théoriques sur équation de Bellman (2 points + 1 point bonus)}
\paragraph{Question 1. } {Principe du calcul de  tous les $\sum_{i=0}^{j-1} p_i$ en $O(1)$ :  } 
\paragraph{Question 2.} Pour  un programme récursif sans calcul redondant implémentant la formule récursive:
travail (nombre d'opérations) $W(n)= \Theta( ... ) $ 
\hfill espacce mémoire $S(n) = \Theta ( ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ) $.
\paragraph{Question 3.} Justification  $r(i,j-1) \leq r(i,j) \leq r(i+1,j)$ :
\\
Equation de Bellman associée:  \\
Coût associé (bonus de 1 point): 
